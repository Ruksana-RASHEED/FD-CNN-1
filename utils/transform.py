...
# SOURCE_DATA_PATH = '../data/sensor_data.csv'
SOURCE_DATA_PATH = '../data/dataset/sensor_data.csv'
#DATASET_FALL_PATH = '../data/dataset/fall_data.csv'
# DATASET_ADL_PATH = '../data/dataset/5_upstair_data.csv'
DATASET_FALL_PATH = '../data/dataset/fall_data_abhi.csv'
value_max = 0
value_min = 0
...
def make_dataset():
    ...
    for i in range(num):
        ...
        value_max = max(data.iloc[i:i + 1, 601:1201].values.reshape([600, ]))
        value_min = min(data.iloc[i:i + 1, 601:1201].values.reshape([600, ]))
        transform_data = transform_sensor_data(sensor_data,i,MAKE_FIGURE=False).reshape([1200,])
        for j in range(1200):
